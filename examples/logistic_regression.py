...
# Make dataset.
rs = ns.random.RandomState(1337)
X1 = rs.randn(500, 1) + 5.0
y1 = ns.zeros(shape=(500,), dtype=bool)
X2 = rs.randn(500, 1) + 10.0
y2 = ns.ones(shape=(500,), dtype=bool)
X = ns.concatenate([X1, X2], axis=0)
y = ns.concatenate([y1, y2], axis=0)


# Train Logistic Regression Model.
model = LogisticRegression(ns.app, opt="newton", opt_params={"tol": 1e-8, "max_iter": 1})
model.fit(X, y)
