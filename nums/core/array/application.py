...
class ArrayApplication(object):
    ...
    def compute_block_shape(self,
                            ...
                            cluster_shape=None,
                            num_cores=None):
        return self.cm.compute_block_shape(shape, dtype, cluster_shape,
                                           num_cores)
    ...
    def read_csv(self,
        ...
        result = self.concatenate(arrays,
                                  axis=0,
                                  axis_block_size=block_shape[0])
    ...
    def loadtxt(self,
                ...
                encoding='bytes',
                max_rows=None,
                num_workers=None) -> BlockArray:
        if num_workers is None:
            num_workers = self.cm.num_cores_total()
        return self._fs.loadtxt(fname,
                                dtype=dtype,
                                comments=comments,
                                delimiter=delimiter,
    ...
    def concatenate(self, arrays: List, axis: int, axis_block_size: int = None):
        ...
        result_shape, result_block_shape = tuple(result_shape), tuple(
            result_block_shape)
        result_ba = self.empty(result_shape, result_block_shape,
                               first_arr.dtype)
        ...
        for arr in arrays:
            ...
            result_selector = tuple([slice(None, None) for _ in range(axis)] +
                                    [axis_slice, ...])
    ...
    def linspace(self, start, stop, shape, block_shape, endpoint, retstep,
                 dtype, axis):
        assert axis == 0
    ...
    def var(self, X: BlockArray, axis=None, ddof=0, keepdims=False, dtype=None):
        ...
        num_summed = (np.product(X.shape)
                      if axis is None else X.shape[axis]) - ddof
        res = ss / num_summed
        if dtype is not None:
            res = res.astype(dtype)
        return res
    ...
    def where(self,
        ...
        if x is not None or y is not None:
            ...
            result_block_shape = self.cm.compute_block_shape(
                result_shape, np.int64)
            result_arrays = []
    ...
    def map_bop(self,
        ...
        try:
            ...
            if (op_name.endswith("max") or op_name == "maximum" or
                    op_name.endswith("min") or op_name == "minimum" or
                    op_name.startswith("logical")):
    ...
    def array_compare(self, func_name, a: BlockArray, b: BlockArray, *args):
        ...
        for grid_entry in a.grid.get_entry_iterator():
            a_block, b_block = a.blocks[grid_entry].oid, b.blocks[
                grid_entry].oid
            bool_list.append(
                self.cm.array_compare(func_name,
                                      a_block,
                                      b_block,
                                      args,
                                      syskwargs={
    ...
    def indirect_tsr(self, X: BlockArray, reshape_output=True):
        ...
        tsR = BlockArray(
            ArrayGrid(shape=R_shape,
                      block_shape=R_shape,
                      dtype=X.dtype.__name__), self.cm)
    ...
    def direct_tsqr(self, X, reshape_output=True):
        ...
        for i, grid_entry in enumerate(Q.grid.get_entry_iterator()):
            Q.blocks[grid_entry].oid = self.cm.bop(
                "tensordot",
                Q_oids[i],
                Q2_oids[i],
                a1_T=False,
        ...
        R_shape = (shape[1], shape[1])
        R_block_shape = (block_shape[1], block_shape[1])
        tsR = self._vec_from_oids([R2_oid],
                                  shape=R_shape,
                                  block_shape=R_shape,
                                  dtype=X.dtype)
        # If blocking is "tall-skinny," then we're done.
    ...
    def svd(self, X):
        ...
        R_U: BlockArray = self._vec_from_oids([R_U], R_shape, R_block_shape,
                                              X.dtype)
        S: BlockArray = self._vec_from_oids([S], R_shape[:1], R_block_shape[:1],
                                            X.dtype)
    ...
    def ridge_regression(self, X: BlockArray, y: BlockArray, lamb: float):
        ...
        lamb_vec = self.array(lamb * np.eye(R_shape[0]),
                              block_shape=R_block_shape)
    ...
    def nanmean(self, a: BlockArray, axis=None, keepdims=False, dtype=None):
        ...
        res = self.reduce(
            "nansum", a, axis=axis, dtype=dtype, keepdims=keepdims) / num_summed
    ...
    def nanvar(self,
        ...
        ss = self.reduce("nansum", (a - mean)**self.two,
                         axis=axis,
                         dtype=dtype,
                         keepdims=keepdims)
        num_summed = self.sum(
            ~a.ufunc("isnan"), axis=axis, dtype=a.dtype,
