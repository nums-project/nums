...
class ArrayApplication(object):
    ...
    def compute_block_shape(
        ...
        if cluster_shape is not None:
            pass
        elif isinstance(self.system, RaySystem) and isinstance(
            self.system.scheduler, BlockCyclicScheduler
        ):
    ...
    def write_s3(self, ba: BlockArray, filename: str):
        ...
        result = self._filesystem.write_meta_s3(
            filename,
    ...
    def delete_s3(self, filename: str):
        ...
        result = self._filesystem.delete_meta_s3(
            filename,
            syskwargs={"grid_entry": grid_entry, "grid_shape": grid.grid_shape},
        )
        ...
        results: BlockArray = self._delete(
            filename, StoredArrayS3, self._filesystem.delete_block_s3
        )
        return results
    ...
    def read_csv(
        ...
        arrays: list = self._filesystem.read_csv(
    ...
    def array(self, array: np.ndarray, block_shape: tuple = None):
        ...
        return BlockArray.from_np(
            array, block_shape=block_shape, copy=False, system=self.system
        )
    ...
    def concatenate(self, arrays: List, axis: int, axis_block_size: int = None):
        ...
        result_shape, result_block_shape = (
            tuple(result_shape),
            tuple(result_block_shape),
        )
    ...
    def argop(self, op_name: str, arr: BlockArray, axis=None):
        ...
        for grid_entry in arr.grid.get_entry_iterator():
            ...
            reduction_result = self.system.arg_op(
        ...
        if x is not None or y is not None:
            ...
            for axis in range(num_axes):
                ...
                for i in range(len(result_oids)):
                    ...
                    block_arrays.append(
                        BlockArray.from_oid(
                            result_oids[i][axis], shapes[i], np.int64, self.system
                        )
                    )
        ...
        try:
            ...
            if (
                ...
                rarr = BlockArray.from_blocks(
                    result_blocks, result_shape=None, system=self.system
                )
    ...
    def indirect_tsr(self, X: BlockArray, reshape_output=True):
        ...
        tsR.blocks[0, 0].oid = self.system.qr(
            *R_oids,
            mode="r",
            axis=0,
            syskwargs={
    ...
    def direct_tsqr(self, X, reshape_output=True):
        ...
        # Construct Q.
        Q = self.zeros(
            shape=shape, block_shape=(block_shape[0], shape[1]), dtype=X.dtype
        )
        for i, grid_entry in enumerate(Q.grid.get_entry_iterator()):
            Q.blocks[grid_entry].oid = self.system.bop(
                "tensordot",
                Q_oids[i],
                Q2_oids[i],
                a1_T=False,
        ...
        R_shape = (shape[1], shape[1])
        R_block_shape = (block_shape[1], block_shape[1])
        tsR = self._vec_from_oids(
            [R2_oid], shape=R_shape, block_shape=R_shape, dtype=X.dtype
        )
        # If blocking is "tall-skinny," then we're done.
    ...
    def svd(self, X):
        ...
        S: BlockArray = self._vec_from_oids(
            [S], R_shape[:1], R_block_shape[:1], X.dtype
        )
    ...
    def _inv(self, remote_func, kwargs, X: BlockArray):
        ...
        result.blocks[0, 0].oid = remote_func(
    ...
    def cholesky(self, X: BlockArray):
        ...
        if single_block:
            ...
            result = X.reshape(block_shape=X.shape)
        result.blocks[0, 0].oid = self.system.cholesky(
            result.blocks[0, 0].oid,
            syskwargs={"grid_entry": (0, 0), "grid_shape": (1, 1)},
        )
        if not single_block:
    ...
    def _vec_from_oids(self, oids, shape, block_shape, dtype):
        arr = BlockArray(
            ArrayGrid(shape=shape, block_shape=shape, dtype=dtype.__name__), self.system
        )
