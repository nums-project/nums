...
class BlockArray(BlockArrayBase):
    @classmethod
    def empty(cls, shape, block_shape, dtype, cm: KernelManager):
        return BlockArray.create("empty", shape, block_shape, dtype, cm)

    @classmethod
    def create(cls, create_op_name, shape, block_shape, dtype, cm: KernelManager):
    ...
    def from_scalar(cls, val, cm):
        ...
        return BlockArray.from_np(np.array(val), block_shape=(), copy=False, km=cm)
    ...
    def reshape(self, *shape, **kwargs):
        ...
        shape = Reshape.kernel_shape(self.shape, shape)
    ...
    @staticmethod
    def to_block_array(obj, cm: KernelManager, block_shape=None):
    ...
    def __elementwise__(self, op_name, other):
        ...
        return BlockArray.from_blocks(
            blocks_op(other.blocks), result_shape=None, km=self.cm
        )
...
class Reshape:
    ...
    def _arbitrary_reshape(self, arr: BlockArray, shape, block_shape) -> BlockArray:
        ...
        dst_arr = BlockArray.empty(
            shape=shape, block_shape=block_shape, dtype=arr.dtype, km=cm
        )
        for dst_grid_entry in dst_arr.grid.get_entry_iterator():
            dst_block: Block = dst_arr.blocks[dst_grid_entry]
