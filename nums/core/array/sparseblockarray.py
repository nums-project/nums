...
from nums.core.array.base import SparseBlockArrayBase, SparseBlock
...
class SparseBlockArray(SparseBlockArrayBase):
    ...
    def empty(cls, shape, block_shape, dtype, system):
        ...
        arr = SparseBlockArray(grid, system)
    ...
    def from_scalar(cls, val, system):
        if isinstance(val, int):
            ...
            dtype = None
        return SparseBlockArray.from_np(np.array(val, dtype=dtype),
                                  block_shape=(),
                                  copy=False,
                                  system=system)
    ...
    def from_oid(cls, oid, shape, dtype, system):
        ...
        ba = SparseBlockArray(grid, system)
    ...
    def copy(self):
        ...
        rarr_copy = SparseBlockArray(grid_copy, self.system)
    ...
    def __getattr__(self, item):
        if item == "__array_priority__" or item == "__array_struct__":
            ...
            rarrT = SparseBlockArray(gridT, self.system)
    ...
    def __getitem__(self, item):
        ...
        tmp = []
        for entry in ss:
            if isinstance(entry, SparseBlockArray):
        ...
        return av[item].create(SparseBlockArray)
    ...
    def _advanced_single_array_subscript(self, sel: tuple, block_size=None, axis=0):
        ...
        dst_arr = SparseBlockArray.empty(shape=shape,  block_shape=block_shape,
    ...
    def tensordot(self, other, axes=2):
        ...
        def basic_vector(ba: SparseBlockArray, axis):
            if len(ba.shape) == 0:
                return False
            if len(ba.shape) == 1:
                return True
    ...
    def __inequality__(self, op, other):
        ...
        result = SparseBlockArray(grid, self.system)
...
class Reshape(object):
    ...
    def _arbitrary_reshape(self, arr: SparseBlockArray, shape, block_shape) -> SparseBlockArray:
    ...
    def _block_shape_reshape(self, arr, block_shape):
        rarr: SparseBlockArray = SparseBlockArray.empty(arr.shape, block_shape, arr.dtype, arr.system)
    ...
    def _is_simple_reshape(self, arr: SparseBlockArray, shape, block_shape):
    ...
    def _simple_reshape(self, arr, shape, block_shape):
        ...
        rarr = SparseBlockArray(grid, arr.system)
        for grid_entry in grid.get_entry_iterator():
    ...
    def __call__(self, arr: SparseBlockArray, shape, block_shape):
