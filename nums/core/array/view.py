...
class ArrayView(object):
    ...
    def create_basic_multi_step(self, concrete_cls) -> BlockArrayBase:
        ...
        raise NotImplementedError(
            "Positive steps of size 1 are currently supported.")
    ...
    def basic_assign(self, subscript: tuple, value):
        ...
        dst_sel = selection.BasicSelection.from_subscript(
            dst_ba.shape, subscript)
        ...
        if dst_sel.basic_steps():
            ...
            if isinstance(value, ArrayView):
                value_is_aligned = value.sel.is_aligned(
                    value._source.block_shape)
            if (value_is_aligned and
                    dst_sel.is_aligned(self._source.block_shape) and
                    self.block_shape == value.block_shape):
