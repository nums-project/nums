...
class ArrayView(object):
    ...
    def create_references(self, concrete_cls) -> BlockArrayBase:
        ...
        dst_inflated_block_shape = self.sel.get_broadcastable_block_shape(self.block_shape)
        ...
        for dst_index, dst_inflated_grid_entry in enumerate(dst_inflated_grid.get_entry_iterator()):
            dst_grid_entry = dst_grid_entry_iterator[dst_index]
            src_grid_entry = tuple((np.array(dst_inflated_grid_entry, dtype=np.int)
                                    + grid_offset).tolist())
    ...
    def assign_references(self, dst_sel: BasicSelection, value):
        ...
        dst_offset = dst_sel.position().value // np.array(self._source.block_shape,
                                                          dtype=np.int)
        # Do we need to broadcast?
        if (isinstance(value, ArrayView)
                and (dst_sel.get_output_shape() != value.sel.get_output_shape())):
            value = value.create()
        if isinstance(value, ArrayView):
            # This is the best case.
            # We don't need to create value to perform the reference copy.
            # No broadcasting required, so this should be okay.
            src_offset = value.sel.position().value // np.array(value._source.block_shape,
                                                                dtype=np.int)
            src_inflated_shape = dst_sel.get_broadcastable_shape()
            src_inflated_block_shape = dst_sel.get_broadcastable_block_shape(value.block_shape)
            ...
            for src_grid_entry_inflated in src_inflated_grid.get_entry_iterator():
                ...
                dst_grid_entry = tuple((np.array(src_grid_entry_inflated, dtype=np.int)
                                        + dst_offset).tolist())
                src_grid_entry = tuple((np.array(src_grid_entry_inflated, dtype=np.int)
                                        + src_offset).tolist())
                self._source.blocks[dst_grid_entry] = value._source.blocks[src_grid_entry].copy()
            ...
            for src_index, src_grid_entry_inflated in \
                ...
                dst_grid_entry = tuple((np.array(src_grid_entry_inflated, dtype=np.int)
                                        + dst_offset).tolist())
                self._source.blocks[dst_grid_entry] = src_ba.blocks[src_grid_entry].copy()
