...
from nums.core.kernel.kernel_interface import KernelInterface, RNGInterface
...
class KernelManager(KernelInterface):
    """
    ...
    def init_kernel(self, kernel_module):
        ...
        backend_utils.check_implementation(KernelInterface, kernel_imp)
        if getattr(kernel_module, "RNG", None) is None:
            raise Exception(
                "No random number generator implemented "
                "for kernel module %s" % str(kernel_module)
            )
        ...
        required_methods = inspect.getmembers(
            KernelInterface(), predicate=inspect.ismethod
        )
