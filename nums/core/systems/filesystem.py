...
def write_block_s3(block: Any, filename: AnyStr, grid_entry: Tuple,
                   grid_meta: Dict):
    return np.array(StoredArrayS3(filename, ArrayGrid.from_meta(grid_meta)).put(
        grid_entry, block),
                    dtype=dict)
...
# NumPy API
##############
def loadtxt_block(fname, dtype, comments, delimiter, converters, skiprows,
...
# CSV API
##############
def read_csv_block(filename, file_start, file_end, dtype, delimiter,
                   has_header):
...
class FileSystem(object):
    ...
    def __init__(self, cm: ComputeManager):
        ...
        for func in [
                write_meta_s3, delete_meta_s3, write_block_s3, read_block_s3,
                delete_block_s3, write_meta_fs, read_meta_fs, delete_meta_fs,
                write_block_fs, read_block_fs, delete_block_fs, loadtxt_block,
                read_csv_block
        ]:
    ...
    def write_block_s3(self, block: Any, filename: AnyStr, grid_entry: Tuple,
                       grid_meta: Dict, syskwargs: Dict):
    ...
    def read_block_s3(self, filename: AnyStr, grid_entry: Tuple,
                      grid_meta: Dict, syskwargs: Dict):
    ...
    def delete_block_s3(self, filename: AnyStr, grid_entry: Tuple,
                        grid_meta: Dict, syskwargs: Dict):
    ...
    def loadtxt_block(self, fname, dtype, comments, delimiter, converters,
    ...
    def write_block_fs(self, block: Any, filename: AnyStr, grid_entry: Tuple,
                       grid_meta: Dict, syskwargs: Dict):
    ...
    def read_block_fs(self, filename: AnyStr, grid_entry: Tuple,
                      grid_meta: Dict, syskwargs: Dict):
        return self.cm.call("read_block_fs",
                            filename,
                            grid_entry,
                            syskwargs=syskwargs)

    def delete_block_fs(self, filename: AnyStr, grid_entry: Tuple,
                        grid_meta: Dict, syskwargs: Dict):
        return self.cm.call("delete_block_fs",
                            filename,
                            grid_entry,
                            syskwargs=syskwargs)
    ...
    def delete_meta_fs(self, filename: str):
        oids = []
        for device_id in self.cm.devices():
            oid = self.cm.call("delete_meta_fs",
                               filename,
                               syskwargs={"device_id": device_id})
            oids.append(oid)
        return oids
