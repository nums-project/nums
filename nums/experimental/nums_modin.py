...
def from_modin(df):
    ...
    assert isinstance(df,
                      DataFrame), "Unexpected dataframe type %s" % str(type(df))
    ...
    assert dtype in (float, np.float, np.float32, np.float64, int, np.int,
                     np.int32, np.int64)
    ...
    pd_parts = frame._frame_mgr_cls.map_partitions(frame._partitions,
    ...
    for i in range(grid_shape[0]):
        ...
        if grid_shape[1] == 1:
            ...
            row_ba: BlockArray = app.concatenate(cols,
                                                 axis=1,
                                                 axis_block_size=block_shape[1])
        rows.append(row_ba)
