...
class GLM(object):
    ...
    def fit(self, X: BlockArray, y: BlockArray):
        ...
        X = self._app.concatenate([
            X,
            self._app.ones(shape=(X.shape[0], 1),
        ...
        beta: BlockArray = self._app.zeros((X.shape[1],), (X.block_shape[1],),
                                           dtype=X.dtype)
        tol: BlockArray = self._app.scalar(self._tol)
        max_iter: int = self._max_iter
        if self._penalty == "l2":
            self._lambda_vec = self._app.ones(beta.shape, beta.block_shape,
...
class LogisticRegression(GLM):
    ...
    def predict_proba(self, X: BlockArray) -> BlockArray:
        y_pos = self.forward(X).reshape((X.shape[0], 1),
                                        block_shape=(X.block_shape[0], 1))
        y_neg = 1 - y_pos
...
class PoissonRegression(GLM):
    ...
    def deviance(self, y: BlockArray, y_pred: BlockArray) -> BlockArray:
        return self._app.sum(self._app.two * self._app.xlogy(y, y / y_pred) -
                             y + y_pred)
...
def gd(model: GLM, beta, X: BlockArray, y: BlockArray, tol: BlockArray,
       max_iter: int, lr: BlockArray):
    app = _instance()
    for _ in range(max_iter):
