...
def train_test_split(
    *arrays,
    ...
    for i, r_oid in enumerate(result_oids):
        ...
        results.append(
            BlockArray.from_oid(r_oid, shape=shape, dtype=dtype, km=instance().cm)
        )
    return results
    ...
    class NumsModel:
        def __init__(self, *args, **kwargs):
            ...
            if self.__class__ in _place_on_node_0:
                device_id = instance().km.devices()[0]
            self.actor = instance().km.make_actor(
        ...
        def fit_transform(self, X: BlockArray, y: BlockArray = None):
            ...
            return BlockArray.from_oid(
                r_oid, shape=X.shape, dtype=float, km=instance().cm
            )
        ...
        def predict(self, X: BlockArray):
            ...
            return BlockArray.from_oid(
                r_oid, shape=(X.shape[0],), dtype=predict_dtype, km=instance().cm
            )
        ...
        def score(self, X: BlockArray, y: BlockArray, sample_weight: BlockArray = None):
            ...
            return BlockArray.from_oid(r_oid, shape=(), dtype=float, km=instance().cm)
