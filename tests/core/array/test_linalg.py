...
def test_inv_assumptions(app_inst: ArrayApplication):
    ...
    overwrite_L_inv_res, info = lapack.dtrtri(overwrite_L_inv,
                                              lower=1,
                                              unitdiag=0,
                                              overwrite_c=1)
    ...
    scipy_L_inv, info = lapack.dtrtri(scipy.linalg.cholesky(
        np.asfortranarray(np_Z),
        lower=True,
        overwrite_a=True,
        check_finite=False),
                                      lower=1,
...
def test_rr(app_inst: ArrayApplication):
    ...
    real_X, real_y = BimodalGaussian.get_dataset(100,
                                                 num_features,
                                                 p=0.5,
                                                 theta=real_theta)
    extra_X, extra_y = BimodalGaussian.get_dataset(10,
                                                   num_features,
                                                   p=0.5,
                                                   theta=real_theta)
