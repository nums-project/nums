...
def test_multinomial_logistic(nps_app_inst: ArrayApplication):
    ...
    num_samples, num_features, num_classes = real_X.shape[0], real_X.shape[1], real_y_indices.max()+1
    ...
    y = nps_app_inst.array(real_y, block_shape=(100, 3)) # TODO block shape? iris is 3 classes, and we seem to crash when using less than 3 here. 
    param_set = [
    ...
    for kwargs in param_set:
        runtime = time.time()
        lr_model: MultinomialLogisticRegression = MultinomialLogisticRegression(**kwargs)
        lr_model.fit(X, y)
        runtime = time.time() - runtime
        y_pred = lr_model.predict(X)#.get() TODO we should return a nums object not np
